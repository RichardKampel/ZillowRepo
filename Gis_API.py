# GISGRAPHY API: https://services.gisgraphy.com/static/leaflet/index.html#
...
class FulltextQuery:
        ...
        for parameter_name, parameter_value in self.parameters.items():
            if parameter_counter == 0:
                gisgraphy_url = gisgraphy_url + parameter_name + "=" + \
                                str(urllib.parse.quote((parameter_value)))
            else:
                gisgraphy_url = gisgraphy_url + "&" + parameter_name \
                                + "=" + str(urllib.parse.quote((parameter_value)))
    ...
    def execute(self):
        """execute the query"""
        python_data  = urlopen(self.get_url()).read()
...
def get_latlng(address): #Richard writing this function
    query = FulltextQuery(address)
